...
def scrape_domain(keyword, search_engine_url):
    print(f"Searching for: {keyword}")
    results = []
    count = 0

    try:
        html_content = fetch_url(search_engine_url)
        if html_content:
            soup = BeautifulSoup(html_content.text, 'html.parser')
...
def main():
    ...
    for keyword in keywords:
        ...
        results = scrape_domain(keyword, bing_search_url)
        results.extend(scrape_domain(keyword, yahoo_search_url))
